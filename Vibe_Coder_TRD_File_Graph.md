...
- ✅ A repo with 1,000+ files loads its graph in collapsed (directory-only) form without a noticeable render stall — validated two ways: the full pipeline on `microsoft/TypeScript` (4,905 real files, 6,864 chunks indexed, e2e 12/12), and synthetic metrics (`scripts/filegraph-metrics.mjs`) showing first-paint stays at 11–101 visible nodes for 1k–10k-file trees (worst-case `buildGraph` 15ms on a 100k-file synthetic). Auto-expand is content-bounded (≤12 children and ≤200 files), fixing the original depth-based rule that could fan out hundreds of nodes.
